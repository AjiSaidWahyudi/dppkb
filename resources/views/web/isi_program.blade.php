    ...
    <div class="container column-wrapper">
        <div class="row row-wrapper">
            <div id="main-container" class="col-md-8 main-content ">
                <ol id="breadcrumb" class="breadcrumb"><li><a href="/">Beranda</a></li><li><a href="#">Program</a></li><li class="active">{{$program->nama}}</li></ol>
                <article id="post-391" class="article-content push_bottom_40 post-391 post type-post status-publish format-standard has-post-thumbnail hentry category-transportation tag-aircraft tag-airplanes tag-airport tag-bright">
                    <h2 class="single-article-title">{{$program->nama}}</h2>
                    <ul class="article-meta post-meta list-inline">
                        <li class="post-date"><span>{{$program->created_at}}</span></li>
                    </ul>
                    <div class="post-entry">    
                        <p>{{$program->detail}}</p>
                        <a href="{{route('kegiatan.show', $kegiatan->slug)}}" type="button" target="_blank">{{$kegiatan->nama}}</a>
                    </div>
                </article>
            </div>
